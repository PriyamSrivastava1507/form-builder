...
import type { TextValidation as TextValidationType } from "@/types/fieldValidation"
import isValidRegex from "@/utils/isRegex";
import { useEffect, useRef, useState } from "react";

type TextValidationProps = {
    fieldId: string;
    validations: TextValidationType;
}

const TextValidation = ({fieldId, validations}: TextValidationProps) => {
    ...
    const [localMinLength, setLocalMinLength] = useState<string>(
        validations.minLength?.value!==null ? validations.minLength.value.toString() : ""
    );
    ...
    const [localMaxLength, setLocalMaxLength] = useState<string>(
        validations.maxLength?.value!==null ? validations.maxLength.value.toString() : ""
    );

    const [localMaxErrorMessage, setLocalMaxErrorMessage] = useState<string>(validations.maxLength?.errorMessage);

    const [localRegex, setLocalRegex] = useState<string>(validations.pattern?.value);

    const [localRegexErrorMessage, setLocalRegexErrorMessage] = useState<string>(validations.pattern?.errorMessage);

    const debouncedRegexValidation = useDebounceCallback((value: string) => {
        const valid = isValidRegex(value);
        setIsRegexValid(valid);
        if (valid) {
            updateField(fieldId, { validations: { ...validationsRef.current, pattern: { ...validationsRef.current.pattern, value } }});
        } else {
            setLocalRegex("");
        }
    }, 1000);
    ...
    useEffect(() => {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setLocalRegex(validations.pattern?.value);
    }, [validations.pattern?.value]);

    useEffect(() => {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setLocalRegexErrorMessage(validations.pattern?.errorMessage);
    }, [validations.pattern?.errorMessage]);
    ...
    const handleRegexChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setLocalRegex(e.target.value);
        debouncedRegexValidation(e.target.value);
    }

    const handleRegexErrorMessageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setLocalRegexErrorMessage(e.target.value);
        debouncedUpdate(fieldId, {validations: {...validationsRef.current, pattern: { ...validationsRef.current.pattern, errorMessage: e.target.value }}});
    }
        ...
        <div className="mb-2">
            ...
            <div className="mb-3">
                <div className="flex justify-between items-center mb-2">
                    <p className="text-sm font-semibold text-foreground-muted">Regex Pattern :</p>
                    {
                        !isRegexValid && localRegex=="" && (
                            <p className="text-[0.65rem] text-destructive/80 mt-0.5">Invalid Regex Pattern</p>
                        )
                    }
                </div>
                <div className="py-2 pr-2">
                    <label htmlFor={fieldId+"-regex"} className="block text-xs font-semibold text-foreground-muted mb-2">Pattern :</label>
                    <input 
                        type="text" 
                        id={fieldId+"-regex"} 
                ...
                <div className="py-2 pr-2">
                    <label htmlFor={fieldId+"-regex-error-message"} className="block text-xs font-semibold text-foreground-muted mb-2">Error Message :</label>
                    <input 
                        type="text" 
                        id={fieldId+"-regex-error-message"} 
                        placeholder="Enter error message" 
                        value={localRegexErrorMessage}
                        onChange={handleRegexErrorMessageChange}
                        className="w-full p-1.5 rounded-sm border-b-[1.5px] bg-surface-overlay/60 outline-none text-foreground/80 text-xs border border-border/20 placeholder:text-foreground/60 hover:border-primary/50 focus:border-primary/50 transition-all duration-200" 
                    />
                    <p className="text-xs text-muted-foreground mt-5">* Enter the regex pattern between '/' and '/'. Example: \d{10}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">* Default: No regex, accepts any pattern</p>
                    <p className="text-xs text-muted-foreground mt-0.5">* Here is a website for more information about regex patterns: <a href="https://regex101.com/" target="_blank" rel="noopener noreferrer" className="text-xs text-foreground/80 font-medium underline cursor-pointer hover:text-foreground/90 transition-all duration-150">regex101.com</a></p>
                </div>
            </div>
        </div>
    )
