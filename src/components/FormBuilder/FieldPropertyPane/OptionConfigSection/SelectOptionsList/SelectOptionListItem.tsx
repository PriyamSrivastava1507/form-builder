...
type SelectOptionListItemProps = {
    field: SelectField;
    sortedIndex: number;
    optionIndex: number;
}
const SelectOptionListItem = ({field, optionIndex, sortedIndex}: SelectOptionListItemProps) => {
        ...
        <div className="px-2 py-1 flex gap-2 items-center">
            ...
            <Switch 
                ...
                checked={
                    (!field.options[optionIndex].disabled) && (
                        (field.multiselect)?
                            (field.defaultValue?.includes(field.options[optionIndex].value) ?? false)
                        :
                            (field.defaultValue?.[field.defaultValue.length-1] === field.options[optionIndex].value)
                    )
                }
                onCheckedChange={(checked) => {
                    if(checked){
                        ...
                        if(!field.multiselect){
                            newDefaultValue = [field.options[optionIndex].value];
                        }else{
                            newDefaultValue.push(field.options[optionIndex].value);
                        }
                        updateField(field.id, {defaultValue: newDefaultValue});
                    }else{
                        const newDefaultValue = (field.defaultValue || []).filter((val) => val !== field.options[optionIndex].value);
                        updateField(field.id, {defaultValue: newDefaultValue});
                    }
                }}
            />
